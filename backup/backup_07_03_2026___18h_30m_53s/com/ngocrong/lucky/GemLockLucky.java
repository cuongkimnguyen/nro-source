...
public class GemLockLucky extends Lucky {

    public GemLockLucky(int id, String name) {
        super(id, name);
    }
    ...
    public void join(Player _c, byte type) {
        ...
        if (type == 0) {
            if (totalNormal >= 2000000000) {
                _c.service.sendThongBao("Đã đạt số lượng hồng ngọc tối đa");
                return;
            }
            if (_c.getDiamondLock() < 1) {
                return;
            }
        } else {
    ...
    public void show(Player _player) {
        ...
        _player.menus.add(new KeyValue(782, "Cập nhật"));
        ...
        if (percentNormal[1] != 0) {
            sb.append(String.format("Tổng giải thưởng: %s hồng ngọc, cơ hội trúng hiện tại của bạn là: %d.%d%%\n", Utils.currencyFormat(totalRewardNormal), percentNormal[0], percentNormal[1]));
        } else {
            sb.append(String.format("Tổng giải thưởng: %s hồng ngọc, cơ hội trúng hiện tại của bạn là: %d%%\n", Utils.currencyFormat(totalRewardNormal), percentNormal[0]));
        }
        if (percentVIP[1] != 0) {
            sb.append(String.format("Tổng giải VIP: %s hồng ngọc, cơ hội trúng hiện tại của bạn là: %d.%d%%\n", Utils.currencyFormat(totalRewardVIP), percentVIP[0], percentVIP[1]));
        } else {
            sb.append(String.format("Tổng giải VIP: %s hồng ngọc, cơ hội trúng hiện tại của bạn là: %d%%\n", Utils.currencyFormat(totalRewardVIP), percentVIP[0]));
        }
