 ...
 */
public class TopKillRaiti extends Top {
    ...
    final String query = "SELECT p.id, p.name, p.head2, p.body, p.leg, "
            + "CAST(JSON_EXTRACT(e.point,'$[0]') AS UNSIGNED) AS raiti "
            + "FROM nr_event_tet e JOIN nr_player p ON e.player_id=p.id "
            + "ORDER BY raiti DESC LIMIT 100";

    public TopKillRaiti(int id, byte type, String name, byte limit) {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    info.body = body;
                    info.leg = leg;
                    info.info = String.format("Số lần hạ: %d", point);
                    info.info2 = "";
                    info.score = point;
                    i++;
                    elements.add(info);
