...
public class GoldBarLucky extends Lucky {

    public GoldBarLucky(int id, String name) {
        super(id, name);
    }
    ...
    public void join(Player _c, byte type) {
        ...
        int index = _c.getIndexBagById(ItemName.THOI_VANG);
        Item item = null;
        if (index != -1) {
            item = _c.itemBag[index];
        }
        if (item == null) {
            _c.service.sendThongBao("Bạn không có thỏi vàng");
            return;
        }
        if (type == 0) {
            if (totalNormal >= 2000000000) {
                _c.service.sendThongBao("Đã đạt số lượng thỏi vàng tối đa");
                return;
            }
            if (item.quantity < 1) {
                return;
        ...
        if (pl.numberOfBets < 10) {
            if (type == 0) {
                _c.removeItem(index, 1);
                totalNormal += 1;
                pl.quantity += 1;
            } else {
                _c.removeItem(index, 10);
                totalVip += 10;
                pl.quantity += 10;
            }
            pl.numberOfBets++;
    ...
    public void show(Player _player) {
        ...
        _player.menus.add(new KeyValue(781, "Cập nhật"));
        ...
        if (percentNormal[1] != 0) {
            sb.append(String.format("Tổng giải thưởng: %s thỏi vàng, cơ hội trúng hiện tại của bạn là: %d.%d%%\n", Utils.currencyFormat(totalRewardNormal), percentNormal[0], percentNormal[1]));
        } else {
            sb.append(String.format("Tổng giải thưởng: %s thỏi vàng, cơ hội trúng hiện tại của bạn là: %d%%\n", Utils.currencyFormat(totalRewardNormal), percentNormal[0]));
        }
        if (percentVIP[1] != 0) {
            sb.append(String.format("Tổng giải VIP: %s thỏi vàng, cơ hội trúng hiện tại của bạn là: %d.%d%%\n", Utils.currencyFormat(totalRewardVIP), percentVIP[0], percentVIP[1]));
        } else {
            sb.append(String.format("Tổng giải VIP: %s thỏi vàng, cơ hội trúng hiện tại của bạn là: %d%%\n", Utils.currencyFormat(totalRewardVIP), percentVIP[0]));
        }
