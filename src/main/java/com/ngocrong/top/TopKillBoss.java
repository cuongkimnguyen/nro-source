 ...
 */
public class TopKillBoss extends Top {
    private static Logger logger = Logger.getLogger(TopKillBoss.class);

    final String query = "SELECT p.id, p.name, p.head2, p.body, p.leg, "
            + "CAST(JSON_EXTRACT(e.point,'$[3]') AS UNSIGNED) AS boss "
            + "FROM nr_event_tet e JOIN nr_player p ON e.player_id=p.id "
            + "ORDER BY boss DESC LIMIT 100";

    public TopKillBoss(int id, byte type, String name, byte limit) {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    info.body = body;
                    info.leg = leg;
                    info.info = String.format("Điểm hạ Boss: %d", point);
                    info.info2 = "";
                    info.score = point;
                    i++;
                    elements.add(info);
