 ...
 */
public class TopHopQuaDacBiet extends Top {
    private static Logger logger = Logger.getLogger(TopHopQuaDacBiet.class);

    final String query = "SELECT p.id, p.name, p.head2, p.body, p.leg, "
            + "CAST(JSON_EXTRACT(e.point,'$[10]') AS UNSIGNED) AS boss "
            + "FROM nr_event_tet e JOIN nr_player p ON e.player_id=p.id "
            + "ORDER BY boss DESC LIMIT 100";

    public TopHopQuaDacBiet(int id, byte type, String name, byte limit) {
